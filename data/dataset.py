        ...
        if len(masklist) > 0:
            ...
            if self.opt.mask_content == 'Multiple':
                if mask[0].size != image.size:
                    mask = [m.resize(image.size, resample=Image.NEAREST).crop((roi[2], roi[0], roi[3], roi[1])) for m in mask]
                else:
...
class Resize:
    ...
    def __call__(self, img, mask, has_mask):
        ...
        if isinstance(mask, list):
            mask = [transforms.functional.resize(m, size=self.target_size, interpolation=transforms.InterpolationMode.NEAREST) for m in mask]
        else:
